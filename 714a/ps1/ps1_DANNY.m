...
while mindist > tol && j < 100
    ...
    inccon = (c_ss - cmin)/numcands; % step amount
    c_cand = cmin:inccon:c_ss; % grid 
    ...
    for i = 1:length(c_cand)
        ...
        for t = 2:traj_periods
            ...
            if t == T; knext = knext - D; end % if the earth quake happens,
                                              % we have to consider the
                                              % shock.
            
            cnext = (beta*(alpha*knext^(alpha-1)+1-delta))^(1/sigma)*c;
                    %%% why is it not -1/sigma?
            
            if isreal(knext) && isreal(cnext) && ~isnan(knext) && ~isnan(cnext)
                    %%% put next period into trajectory, but had to do if
                    %%% else because sometimes they were fucked up. if they
                    %%% are real and measurable 
                k_traj(i,t) = knext;
                c_traj(i,t) = cnext;
            else
                k_traj(i, t) = k; %%% go back to old value to converge
                c_traj(i, t) = c;
            end
